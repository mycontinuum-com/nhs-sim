...
test("concurrent sign-ups reserve the final team place atomically", { skip: !process.env.ROW_STORAGE_TEST_URL }, async () => {
  ...
  try {
    ...
    await store.init();
    await store.pool.query("INSERT INTO team_keys(hash,team,world,scopes) SELECT 'fixture-' || n, 'Capacity fixture', 'default', '[\"gp\"]'::jsonb FROM generate_series(1,4999) n");
    store.keys = (await store.pool.query("SELECT hash,team,world,scopes FROM team_keys")).rows;
