...
Draft a prescription with `draft_prescription` from GP or hospital. Its owner is pharmacy. GP can retrieve its prescriptions at `/api/sites/gp/prescriptions` and perform `review` then `accept` through `/api/sites/gp/actions` for prescriptions originally drafted by GP. Send the latest `expectedVersion` with each GP change. Read the product catalogue at `/api/sites/pharmacy/pharmacy-workspace`. Use `link_prescription_stock` to choose a matching `productId` and `quantity` in units. If the prescription has not been approved, use `/api/sites/pharmacy/actions` to `review` then `accept` it. For an approved prescription, continue with `dispense` then `collect` through that endpoint. Send its current `expectedVersion` with each action. Acceptance changes it to `approved`; dispensing deducts the linked quantity from stock. Use a team key with pharmacy scope.
...
2. Open Hospital EPR from the map. Select Amira and find your referral in **Handover**.
