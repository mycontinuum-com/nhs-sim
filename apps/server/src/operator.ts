...
const publicSegments = new Set('api browser sites nhs control teams activity catalogue openapi.json team clock session plan-lab keys gp hospital community pharmacy diagnostics referrals wearables patient legacy view patients actions appointments attendances pharmacy-workspace documents messaging-workspace devices readings consultations genomes population publish attach worlds incidents agents model-propose snapshot pds ods Patient Organization OrganizationAffiliation Practitioner PractitionerRole Observation Encounter MedicationRequest ServiceRequest DocumentReference Communication Task Bundle metadata'.split(' '));
