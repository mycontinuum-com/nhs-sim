...
const page = (
  title: string,
  content: string,
) => `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escape(title)} · Staff Identity · Care Identity simulator</title><style>
...
</style></head><body><a class="skip" href="#main">Skip to main content</a><header class="masthead"><div class="header-inner"><div class="brand"><img src="/control/brands/staff-identity.svg" alt="" width="52" height="52" style="vertical-align:middle;margin-right:12px">Staff Identity <span class="badge">SIMULATOR</span></div><a href="/control/">Back to the neighbourhood</a></div></header><div class="simulation"><div class="strip-inner"><strong>SIMULATION</strong> · Fictional staff identities. This is not NHS authentication.</div></div><main id="main"><div class="service">${content}</div></main><footer class="footer"><div>A local care identity simulation for NHS-SIM. No real smartcard, security key or staff credentials are used.</div></footer></body></html>`;
const developerTools = `<details class="technical"><summary>Developer and operator tools</summary><h2>Connect your application</h2><p>Client <code>nhs-sim-client</code><br>Registered callback <code>/cis2/callback</code></p><p><a href="/cis2/.well-known/openid-configuration">Open the discovery document</a></p><p class="hint">Authorization code with PKCE S256 and RS256 identity tokens. Sessions and settings reset when the simulator restarts.</p></details><section class="operator" id="operator-section"><h2>Organiser identity controls</h2><p>Change the sign-in scenario for every team on this simulator. Applying settings revokes existing staff sessions.</p><p><a href="/control/?operator=1">Open organiser controls to explore teams and their activity</a></p><form id="operator-login"><label class="field"><span>Operator token</span><input id="operator" type="password" autocomplete="off" required aria-describedby="operator-hint"></label><p class="hint" id="operator-hint">Paste the organiser token for this website, with or without “Bearer”. A team API key cannot unlock these controls. The verified token is remembered in this tab until you disconnect or close it.</p><button class="secondary" id="load" type="submit">Connect as organiser</button></form><div id="operator-connected" hidden><p>Connected as organiser for this website.</p><button class="secondary" id="disconnect" type="button">Disconnect organiser</button></div><div id="controls" hidden><label class="field"><span>Sign-in scenario</span><select id="scenario"><option value="normal">Normal sign-in</option><option value="deny">Consent denied</option><option value="unavailable">Service unavailable</option><option value="expired-session">Session expired</option></select></label><label class="field"><span>Token lifetime in seconds</span><input id="lifetime" type="number" min="30" max="3600"></label><details class="technical"><summary>Registered applications</summary><p>Each application has an id, name and redirectUris array. Callback URLs must match exactly.</p><textarea id="clients" rows="10" aria-label="Registered applications JSON"></textarea></details><div class="actions"><button class="primary" id="save">Apply settings</button><button class="secondary" id="revoke">Revoke all sessions</button></div></div><p id="status" role="status" aria-live="polite"></p></section>`;
const landing = () =>
  page(
    "Sign in",
...
const operatorStorageKey='sim-operator-token';
let operatorToken=sessionStorage.getItem(operatorStorageKey)||'';
function disconnectOperator(){
  operatorToken='';sessionStorage.removeItem(operatorStorageKey);$('operator').value='';
  $('controls').hidden=true;$('operator-connected').hidden=true;$('operator-login').hidden=false;
}
async function operator(method,body,token=operatorToken){
  const response=await fetch('/api/operator/cis2',{method,headers:{Authorization:'Bearer '+token,'Content-Type':'application/json'},...(body?{body:JSON.stringify(body)}:{})});
  const data=await response.json();
  if(!response.ok){
    if(response.status===401||response.status===403){disconnectOperator();throw Error('This operator token was not accepted by '+location.host+'. Paste the organiser token configured for this website. A team API key will not work.');}
    throw Error(data.message||data.error||'Could not load identity controls. Try again.');
  }
  return data;
}
function show(data){
  $('controls').hidden=false;$('operator-connected').hidden=false;$('operator-login').hidden=true;
  $('scenario').value=data.scenario;$('lifetime').value=data.tokenLifetimeSeconds;
  $('clients').value=JSON.stringify(data.clients,null,2);
  $('status').textContent='Scenario: '+data.scenario+'. '+data.active.tokens+' active identity tokens. Settings affect every team.';
}
async function connectOperator(token){
  $('load').disabled=true;$('status').textContent='Checking organiser access…';
  try{const data=await operator('GET',undefined,token);operatorToken=token;sessionStorage.setItem(operatorStorageKey,token);$('operator').value='';show(data);}
  catch(error){$('status').textContent=error instanceof TypeError?'Could not reach the simulator. Check your connection and try again.':error.message;}
  finally{$('load').disabled=false;}
}
$('operator-login').addEventListener('submit',event=>{
  event.preventDefault();const token=$('operator').value.trim().replace(/^Bearer\\s+/i,'').trim();
  if(!token){$('status').textContent='Enter the operator token for this website.';return;}
  void connectOperator(token);
});
$('disconnect').onclick=()=>{disconnectOperator();$('status').textContent='Organiser disconnected. Your team connection is unchanged.';};
for(const [id,run]of [['save',()=>operator('PUT',{scenario:$('scenario').value,tokenLifetimeSeconds:Number($('lifetime').value),clients:JSON.parse($('clients').value)})],['revoke',()=>operator('DELETE')]]){
  $(id).onclick=async()=>{ $('save').disabled=true;$('revoke').disabled=true;$('status').textContent='Applying identity settings…';try{show(await run());}catch(error){$('status').textContent=error instanceof SyntaxError?'Registered applications must contain valid JSON. Your settings have not changed.':error.message;}finally{$('save').disabled=false;$('revoke').disabled=false;} };
}
if(new URLSearchParams(location.search).has('operator'))$('operator-section').scrollIntoView();
if(operatorToken)void connectOperator(operatorToken);
document.querySelectorAll('[name=signin_method]').forEach(input=>input.addEventListener('change',()=>{$('start').textContent='Simulate '+(input.value==='security-key'?'security key':'smartcard')+' sign-in ›'}));$('start-form').addEventListener('submit',async event=>{event.preventDefault();const signInMethod=new FormData(event.currentTarget).get('signin_method')||'smartcard';$('start').disabled=true;try{const b64=b=>btoa(String.fromCharCode(...b)).replaceAll('+','-').replaceAll('/','_').replaceAll('=','');const verifier=b64(crypto.getRandomValues(new Uint8Array(32))),state=b64(crypto.getRandomValues(new Uint8Array(24))),nonce=b64(crypto.getRandomValues(new Uint8Array(24)));sessionStorage.setItem('cis2-demo',JSON.stringify({verifier,state}));const challenge=b64(new Uint8Array(await crypto.subtle.digest('SHA-256',new TextEncoder().encode(verifier))));location.href='/cis2/authorize?'+new URLSearchParams({client_id:'nhs-sim-client',redirect_uri:location.origin+'/cis2/callback',response_type:'code',scope:'openid profile',code_challenge_method:'S256',code_challenge:challenge,state,nonce,signin_method:signInMethod})}catch(e){$('start-status').textContent=e.message;$('start').disabled=false}});
</script>`,
  );
  ...
  try {
    if (path === "/api/operator/cis2") {
      if (!admin || !req.headers.authorization?.startsWith("Bearer "))
        throw new Cis2Error("unauthorized", "An organiser token is required. Open /cis2/?operator=1 to connect using the token for this website.", 401);
