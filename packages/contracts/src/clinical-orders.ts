...
export const bloodTestOrderSchema = z.object({ panel: field, specimen: field, priority: z.enum(["routine", "urgent"]), collection: z.enum(["now", "next-round"]), clinicalDetails: z.string().trim().min(1).max(2000) });
