...
export function DocumentCoding({ resource, pending, save }: { resource: Resource; pending: boolean; save: (action: Action) => void }) {
  ...
  const matches = query.trim() ? documentSnomedConcepts.filter(concept => !codes.some(item => item.code === concept.code) && `${concept.code} ${concept.display}`.toLowerCase().includes(query.trim().toLowerCase())) : [];
  return <details className="document-coding"><summary>Tags and SNOMED codes <small>{doc.tags.length} {doc.tags.length === 1 ? "tag" : "tags"} · {doc.snomedCodes.length} {doc.snomedCodes.length === 1 ? "code" : "codes"}</small></summary>
    ...
    <ul className="document-code-results" aria-label="SNOMED search results">{matches.map(concept => <li key={concept.code}><button disabled={pending || codes.length >= 20} onClick={() => { setCodes([...codes, concept]); setQuery(""); }}>{concept.display}<small>{concept.code} · Add</small></button></li>)}</ul>{query.trim() && matches.length === 0 && <p>No matching unselected concepts.</p>}
