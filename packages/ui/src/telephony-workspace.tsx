...
export function TelephonyWorkspace({ apiKey, teamName }: { apiKey: string; teamName: string }) {
  ...
  return <section className="telephony-workspace" aria-label="Reception Calls reception desk">
        ...
        <main className="tel-main">
          <div className="tel-detail">
            {selected ? <>
              ...
              <section className="tel-patient"><div className="tel-section-heading"><h3>Matched patient</h3><span>GP Records patient record</span></div><div className="tel-patient-row"><span className="tel-initials">{selected.patientName.split(" ").map(part => part[0]).slice(0, 2).join("")}</span><div><strong>{selected.patientName}</strong><span>Patient ID: {selected.patientId}</span></div><a className="tel-record-link" href={recordUrl(selected)} target="GP Records" onClick={event => { event.preventDefault(); prepareRecord(selected); }}>Open in GP Records <ArrowSquareOut size={20} /></a></div></section>
