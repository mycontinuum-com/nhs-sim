...
function WorldApp({ siteId }: { siteId: SiteId }) {
  ...
  useEffect(() => { if (isMessages) document.title = (siteId === "gp" ? "Messagey" : "Messages") + " | NHS-SIM"; }, [isMessages, siteId]);
  useEffect(() => { if (isDocuments) document.title = "Document Inbox | NHS-SIM"; }, [isDocuments]);
        ...
        <>
          {!key ? (
            <main className="access-gate">
              ...
              <h1>{isTelephony ? practiceApps.telephony.name : isMessages ? (siteId === "gp" ? "Messagey" : "Messages") : isDocuments ? "Document Inbox" : sites.find((s) => s.id === siteId)?.name}</h1>
