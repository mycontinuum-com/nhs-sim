...
const headers={Authorization:'Bearer '+process.env.SIM_API_KEY,'Content-Type':'application/json'};
...
for(const r of view.resources.filter(r=>r.kind==='request'&&r.status==='open')){
  const action={type:'create_task',patientId:r.patientId,title:'Review unresolved request '+r.id};
  if(process.env.APPLY!=='1'){console.log('Proposed:',action);continue;}
  const result=await fetch(origin+'/api/sites/gp/actions',{method:'POST',headers:{...headers,'Idempotency-Key':'loop-closer-'+r.id},body:JSON.stringify(action)});
